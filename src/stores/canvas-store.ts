...
export const useCanvasStore = create<CanvasState>()((set, get) => ({
  ...
  getConnectedInputs: (nodeId) => {
    ...
    // Build structured imageInputs from reference edges
    const targetNode = nodes.find((n) => n.id === nodeId);
    const targetData = targetNode?.data as ImageGeneratorNodeData | undefined;
    const userLabels = targetData?.imageLabels || {};
    const userRoles = targetData?.imageRoles || {};
    const imageInputs: Record<string, ImageInput> = {};
    const usedLabels = new Set<string>();

    const sanitizeLabel = (name: string): string => {
      return name
    ...
    const getUniqueLabel = (baseLabel: string): string => {
      let label = baseLabel;
      let counter = 2;
    ...
    const allRefEdges = [...referenceHandleEdges, ...legacyRefEdges].filter(Boolean);
    for (const edge of allRefEdges) {
      if (!edge) continue;
      const sourceNode = nodes.find((n) => n.id === edge.source);
      if (!sourceNode) continue;
      const url = getImageUrl(sourceNode, edge.sourceHandle);
      if (!url) continue;

      const sourceId = sourceNode.id;
      // Use user-set label, or derive from source node name/data
      let baseLabel: string;
      if (userLabels[sourceId]) {
        baseLabel = userLabels[sourceId];
      } else {
        const nodeName = (sourceNode.data as { name?: string })?.name;
        // Only use filename if it looks human-readable (not a random/generated ID)
        let fileName = '';
        if (sourceNode.type === 'media') {
          const mediaUrl = (sourceNode.data as MediaNodeData)?.url || '';
          const rawName = mediaUrl.split('/').pop()?.split('?')[0] || '';
          const nameWithoutExt = rawName.replace(/\.[^.]+$/, '');
          // Skip names that look auto-generated: short random IDs, hex strings, or prefixed IDs
          const looksAutoGenerated =
            !nameWithoutExt
            || nameWithoutExt.length < 3
            || /^[a-f0-9-]{8,}$/i.test(nameWithoutExt)
            || /^(img|image|media|file|upload|asset)[_-]/i.test(nameWithoutExt)
            || /[a-z0-9]{6,}$/i.test(nameWithoutExt.replace(/^[a-z]+[_-]/i, ''));
          if (!looksAutoGenerated) {
            fileName = rawName;
          }
        }
        baseLabel = sanitizeLabel(nodeName || fileName || 'image');
      }

      const label = getUniqueLabel(baseLabel);
      const role = userRoles[sourceId] || 'reference';
      imageInputs[label] = { role, label, urls: [url], sourceNodeId: sourceId };
    }
